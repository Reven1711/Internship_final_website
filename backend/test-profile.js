...
async function testProfileData() {
  try {
    ...
    // Test 1: Check chemical-frontend index for profile data
    console.log("\n=== Test 1: chemical-frontend index ===");
    try {
      const index1 = pinecone.index("chemical-frontend");
      const dummyVector1 = new Array(1024).fill(0);
      dummyVector1[0] = 1;
      ...
      console.log(
        "chemical-frontend query response:",
        JSON.stringify(queryResponse1, null, 2)
      );
    } catch (error) {
      console.log("Error querying chemical-frontend:", error.message);
    }
    ...
    try {
      ...
      const dummyVector3 = new Array(1024).fill(0);
      dummyVector3[0] = 1;
